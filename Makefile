...
create-dev-certs: ## Create self-signed development certificates (for HTTPS in development, to test docker setup locally). Note that browsers will show a warning when using self-signed certificates, which is expected. You can usually click "Advanced" and then "Proceed to localhost (unsafe)" to bypass the warning when testing locally.
	...
	openssl req -x509 -newkey rsa:2048 -keyout ./nginx/dev-certs/localhost.key -out ./nginx/dev-certs/localhost.crt -days 365 -nodes -subj "/CN=localhost"
	openssl dhparam -out ./nginx/dev-certs/ssl-dhparams.pem 2048
	curl -o ./nginx/certbot-files-for-dev/options-ssl-nginx.conf https://raw.githubusercontent.com/certbot/certbot/refs/heads/main/certbot-nginx/src/certbot_nginx/_internal/tls_configs/options-ssl-nginx.conf
	curl -o ./nginx/certbot-files-for-dev/LICENSE.txt https://raw.githubusercontent.com/certbot/certbot/refs/heads/main/LICENSE.txt
